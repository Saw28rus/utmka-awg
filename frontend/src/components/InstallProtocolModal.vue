...
const portHint = computed(() => {
  ...
  if (id === 'awg31') {
    return 'UDP-порт. Приложение AmneziaVPN 5.x ставит 55424. Нужен клиент 5.0.1.5+. Если 2.0 уже на 55424 — укажи другой порт. 443 свободен — ещё лучше (похоже на QUIC).'
  }
...
const resultHint = computed(() => {
  ...
  if (id === 'awg2' || id === 'awg31' || id === 'awg_legacy') {
    return 'Готово. Добавляй клиентов во вкладке «Клиенты» — ключи генерируются на сервере. Для 3.1 нужен Amnezia VPN 5.0.1.5+.'
  }
