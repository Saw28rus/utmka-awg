...
**Это не просто панель, а мини-экосистема для собственного личного VPN на AmneziaWG 2.0 и 3.1.**
...
### 🔑 Серверы и клиенты
Подключайте несколько VPS, панель сама определит или поставит **AmneziaWG 2.0
и 3.1** (можно оба на одном сервере). Создание профилей и выдача ключей —
в пару кликов, с продлением срока, лимитами и **накопительным учётом трафика**
(счётчик не сбрасывается при перезапуске интерфейса).

Ключ **3.1** открывается в **Amnezia VPN 5.0.1.5 или новее** (вкладка AmneziaVPN,
ссылка `vpn://`). Старое приложение AmneziaWG и клиенты 4.x — только 2.0.
...
Каскад работает на **AmneziaWG 2.0, 3.1** (по отдельности или оба сразу)
и на **Xray (VLESS-Reality)** — маршрутизация серверная, правило действует
для любого клиента:
...
2. В панели: **Настройки → Домен и чат** (хост панели) либо сервер → вкладка
   **«Безопасность»**, если панель стоит на том же VPS, что и VPN.
3. Проверить DNS → выпустить сертификат.
4. Готово: вход по `https://panel.example.com`, а прямой `:8080` можно закрыть.
...
HTTPS и чат для **самой панели** (когда она стоит отдельно от VPN-серверов) —
в панели: **Настройки → Домен и чат**. Там же sslip.io, без добавления этого
VPS в список серверов.
